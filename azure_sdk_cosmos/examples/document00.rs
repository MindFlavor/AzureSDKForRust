...
async fn main() -> Result<(), Box<dyn Error>> {
    ...
    let collection = {
        let collections = client
            .clone()
            .with_database(database.id.to_owned())
            .list_collections()
            .execute()
            .await?;
    ...
    let database_client = client.clone().with_database(database.id.to_owned());
    ...
    client
        .clone()
        .with_database(DATABASE.to_owned())
        .with_collection(COLLECTION.to_owned())
        .delete_collection()
        .execute()
        .await?;
